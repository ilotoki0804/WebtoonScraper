...
* fetch_episode_information: 필수적으로 override되어야 합니다. 이 함수는 최소한 self.episode_titles(각 에피소드의 이름, 리스트), self.episode_ids(각 에피소드의 ID)가 생성되어야 합니다. `reload_manager` 데코레이터로 반드시 감싸야 합니다.
...
* get_information: information.json에 더 추가할 내용이 있다면 추가하세요.
