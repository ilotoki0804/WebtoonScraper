...
| 플랫폼 이름 | 플랫폼 코드 | 스크래퍼 이름 | 웹툰 ID로 사용되는 타입 | 예시 | 추가 인증 요구 여부 |
|--|--|--|--|--|--|
| 네이버 웹툰   | `'naver_webtoon'`   | NaverWebtoonScraper    | int | `809590` | 아니오 |
| webtoons.com | `'webtoons_english'`| WebtoonsEnglishScraper | int | `5291` | 아니오 |
| 버프툰        | `'bufftoon'`        | BufftoonScraper        | int | `1001216` | 예 |
| 네이버 포스트 | `'naver_post'`      | NaverPostScraper       | tuple[int, int] | `(597061, 19803452)` | 아니오 |
| 네이버 게임   | `'naver_game'`      | NaverGameScraper       | int | `5` | 아니오 |
| 레진코믹스    | `'lezhin_comics'`   | LezhinComicsScraper    | str | `"dr_hearthstone"` | 예 |
| 카카오페이지  | `'kakaopage'`       | KakaopageScraper       | int | `53397318` | 아니오 |
| 네이버 블로그 | `'naver_blog'`      | NaverBlogScraper       | tuple[str, int] | `("bkid4", 55)` | 아니오 |
| 티스토리     | `'tistory'`          | TistoryScraper         | tuple[str, str] | `("doldistudio", "진돌만화")` | 아니오 |
...
이 뒤부터는 `webtoon` 명령어를 사용합니다. 만약 오류가 난다면 모든 `webtoon`를 `python -m WebtoonScraper`로 바꿔서 진행하세요. 그래도 비슷한 오류가 반복된다면 path를 포함한 파이썬 설치가 제대로 되었는지, 패키지 설치가 제대로 되었는지, 가상 환경 내에서만 설치된 것은 아닌지 확인하세요.
...
각각에 대해서는 각자의 플랫폼에 대해서는 각자 안내됩니다.
...
만약 현재 디렉토리에 다운로드하고 싶다면 값을 `.`으로 보내세요.
...
Downloading has started(mock=False, show_detailed_error=False, webtoon_id=809590, platform='naver_webtoon', merge_number=None, cookie=None, bearer=None, range=None, download_directory=WindowsPath('webtoon'), list_episodes=True, get_paid_episode=False, subparser_name='download').
 Episode
...
* slice: slice 객체를 값으로 보낼 수도 있습니다. 예를 들어 `slice(2, 11, 3)`으로 다운로드하면 2, 5, 8화가 다운로드됩니다. 이때 11화는 포함되지 **않는**다는 것에 주의하세요.
...
```python
from WebtoonScraper.scrapers import WebtoonsEnglishScraper

# 예시 웹툰 URL: https://www.webtoons.com/en/fantasy/the-top-dungeon-farmer/list?title_no=5656
scraper = WebtoonsEnglishScraper(5656)
scraper.download_webtoon()
```
