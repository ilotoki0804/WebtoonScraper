...
#### -p platform, --platform platform 옵션

`-p platform`과 같이 사용하여 특정 플랫폼에서 사용된다는 점을 알릴 수 있습니다.

플랫폼 지정자는 위의 `웹툰 플랫폼` 절에서 확인할 수 있는데, 예를 들어 네이버 웹툰의 경우에는 `-p naver_webtoon`, 버프툰의 경우에는 `-p bufftoon`이 됩니다.

축약형 또한 사용할 수 있습니다. 예를 들어 `-p naver_webtoon`은 `-p nw`으로 짧게 표현이 가능합니다.
축약형은 직관적이지는 않을 수 있습니다. `webtoon download --help`로 축약형들을 확인해 보거나 위의 `웹툰 플랫폼` 절에서 확인해 보세요.

URL이 아닌 경우 이 플랫폼 인자는 생략할 수 없습니다.

플렛폼은 URL일 경우에는 명시할 필요가 없지만 `-p url`으로 url을 사용하도록 강제할 수 있습니다.
...
이미지를 기준에 따라 세로로 연결할 수 있습니다.

기준은 다음과 같이 네 종류로 나뉩니다.

* --all: 한 에피소드 내에 있는 모든 이미지를 하나의 파일로 연결합니다.
* --count <number>: 한 에피소드 내이 있는 이미지를 <number>개씩 연결합니다. 자연수를 인자로 받습니다.
* --height <number>: 한 에피소드 내에 있는 이미지를 <number>의 높이가 되도록 연결합니다. 자연수를 인자로 받습니다.
* --ratio <ratio>: 한 에피소드 내에 있는 이미지의 세로:가로 비가 <ratio>가 되도록 연결합니다. 실수를 인자로 받습니다.

이 커맨드는 프로그램이 특정한 크기의 이미지 파일을 잘 처리하는 경우 유용하게 사용할 수 있습니다.

* 마루 뷰어의 경우에는 --height를 8000으로 선택했을 때 로딩이 매우 빨라집니다.
...
혹은 `Scraper.from_url`을 통해 URL로부터 Scraper를 initialize할 수 있습니다.

```python
from WebtoonScraper.scrapers import NaverWebtoonScraper

scraper = NaverWebtoonScraper.from_url("https://comic.naver.com/webtoon/list?titleId=809590")
scraper.download_webtoon()
```
...
웹툰을 볼 수 있는 HTML 페이지인 `webtoon.html`는 기본적으로 웹툰 디렉토리에 추가됩니다. 컴퓨터에서 이를 이용해 쉽게 웹툰을 볼 수 있습니다.
...
scraper = WebtoonsDotcomScraper.from_url("https://www.webtoons.com/en/fantasy/the-top-dungeon-farmer/list?title_no=5656")
scraper.download_webtoon()
```
...
scraper = NaverPostScraper.from_url("https://m.post.naver.com/my/series/detail.naver?seriesNo=648552&memberNo=3395565")
scraper.download_webtoon()
```

#### 네이버 포스트 다운로드 시 주의사항

* 네이버 포스트에는 글과 그림이 같이 있을 수 있지만 사진만 다운로드됩니다.
...
```console
webtoon download 5 -p naver_game
```

```console
webtoon download 5 -p ng
```
...
scraper = NaverGameScraper.from_url("https://game.naver.com/original_series/5")
scraper.download_webtoon()
```
...
```console
webtoon download "https://www.lezhin.com/ko/comic/dr_hearthstone" --bearer "YOUR BEARER HERE"
```
...
1. 다음과 같이 다운로드할 수 있습니다.
    ...
    ```console
    WebtoonScraper 1007888 --bearer "YOUR BEARER HERE"
    ```
...
```console
WebtoonScraper 1007888 --bearer "YOUR BEARER HERE" --get-paid-episode
```
...
scraper = KakaopageScraper.from_url("https://page.kakao.com/content/53397318")
scraper.download_webtoon()
```
...
scraper = NaverBlogScraper.from_url("https://m.blog.naver.com/bkid4?categoryNo=55")
scraper.download_webtoon()
```
...
scraper = TistoryScraper.from_url("https://doldistudio.tistory.com/category/진돌만화")
scraper.download_webtoon()
```
...
```console
webtoon download "https://webtoon.kakao.com/content/국민사형투표/1180"
```
...
scraper = KakaoWebtoonScraper.from_url("https://webtoon.kakao.com/content/국민사형투표/1180")
scraper.download_webtoon()
```
