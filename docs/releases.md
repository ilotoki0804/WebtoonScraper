...
* 여러 오타를 수정하고 명명을 개선했습니다. 이 과정에서 일부 API가 변경되었습니다.
* 선택적 의존성 `concat`을 추가했습니다.
* 이미 다운로드된 파일이 있는 경우의 다운로드 속도가 크게 개선되었습니다.
* 그 외 여러 크고 작은 개선이 있었습니다.
* `webtoon.html`가 리눅스/맥에서 이미지가 제대로 정렬되지 않는 오류를 수정했습니다.
* 의존성 버전이 증가하였습니다.
